...
class WorkerGitInterfaceable(Worker):
    def __init__(self, worker_type, config={}, given=[], models=[], data_tables=[], operations_tables=[], platform="github"):
        ...
        self.specs = {
            ...
            # host + port worker is running on (so broker can send tasks here)
    ...
    def find_id_from_login(self, login, platform='github'):
        ...
        self.logger.info(
            "Primary key inserted into the contributors table: " + str(result.inserted_primary_key))
    ...
    def init_oauths(self, platform='github'):
        ...
        for oauth in [{'oauth_id': 0, 'access_token': self.config[key_name]}] + json.loads(
            pd.read_sql(oauthSQL, self.helper_db, params={}
            ...
            self.logger.debug(
                "Found OAuth available for use: {}".format(self.oauths[-1]))
        ...
        if platform == 'github':
            ...
            self.headers = {
                'Authorization': 'Bearer %s' % self.oauths[0]['access_token']}

        self.logger.info("OAuth initialized\n")

    def enrich_cntrb_id(
        self, data, key, action_map_additions={
            'insert': {'source': [], 'augur': []}},
        ...
        if not len(data):
            self.logger.info(
                f"Enrich contrib data is empty for {len(data)}, for the key {key}.")

            raise ValueError

        self.logger.info(
            f"Enriching contributor ids for {len(data)} data points...")
        ...
        table_values_cntrb = self.db.execute(
            s.sql.select(self.get_relevant_columns(
                self.contributors_table, cntrb_action_map))
        ).fetchall()

        source_data = expanded_source_df.to_dict(orient='records')

        # Filter out bad data where we can't even hit the api.
        source_data = [data for data in source_data if f'{prefix}login' in data and data[f'{prefix}login'] != None and type(
            data[f'{prefix}login']) is str]

        self.logger.info(
            f"table_values_cntrb keys: {table_values_cntrb[0].keys()}")
        ...
        for index, data in enumerate(source_data):
            ...
            try:
                ...
                self.print_traceback(
                    "Enrich_cntrb_id, data doesn't have user.id. Using node_id instead", e, True)
                ...
                for row in table_values_cntrb:
                    try:
                        ...
                        self.logger.info(
                            f"Error adding gh_node_id: {e}. Row: {row}")
            ...
            if source_data_id in user_unique_ids:
                ...
                try:
                    ...
                    user_id_row = list(
                        filter(lambda x: x['gh_user_id'] == source_data_id, table_values_cntrb))[0]
                    ...
                    user_id_row = list(
                        filter(lambda x: x['gh_node_id'] == source_data_id, table_values_cntrb))[0]
                ...
                self.logger.info(
                    f"cntrb_id {data['cntrb_id']} found in database and assigned to enriched data")

            # contributor is not in the database
            else:

                self.logger.info(
                    "{} not in database, making api call".format(source_data_id))

                self.logger.info("login: {}".format(data[f'{prefix}login']))

                try:
                    url = ("https://api.github.com/users/" +
                           str(data[f'{prefix}login']))
                except Exception as e:
                    self.logger.info(
                        f"Error when creating url: {e}. Data: {data}")
                ...
                try:
                    ...
                    self.logger.info(
                        f"Contributor was unable to be added to table! Attempting to get cntrb_id from table anyway because of possible collision. Error: {e}")
                    # pass # added by sean 11/29/2021 ... think it might be blocking comment insertion otherwise
                    continue
                ...
                if len(cntrb_id_row) == 1:
                    ...
                    self.logger.error(
                        "Couldn't find contributor in database. Something has gone very wrong. Augur ran into a contributor that is unable to be inserted into the contributors table but is also not present in that table.")
                else:
                    self.logger.info(
                        f"There are more than one contributors in the table with gh_user_id={cntrb['gh_user_id']}")

                cntrb_data = {
                    # came through as a float. Fixed 11/28/2021, SPG
                    'cntrb_id': int(data['cntrb_id']),
                    'gh_node_id': cntrb['gh_node_id'],
                    # NaN user issue. Fixed 11/28/2021, SPG
                    'cntrb_login': str(cntrb['cntrb_login']),
                    # came through as a float. Fixed 11/28/2021, SPG
                    'gh_user_id': int(cntrb['gh_user_id'])
                }
        ...
        for data in source_data:
            ...
            self.logger.debug("User login type: " + str(
                type(data[f'{prefix}login'])) + ". Login: " + str(data[f'{prefix}login']))

            try:
                data['cntrb_id']
            except:
                self.logger.debug(
                    f"AB ERROR: data exiting enrich_cntrb_id without cntrb_id, login is: " + str(data[f'{prefix}login']))
    ...
    def query_github_contributors(self, entry_info, repo_id):
        ...
        """
        self.logger.info(
            f"Querying contributors with given entry info: {entry_info}\n")
        ...
        contributors = self.paginate(
            contributors_url, duplicate_col_map, update_col_map, table, table_pkey)

        self.logger.info(
            "Count of contributors needing insertion: " + str(len(contributors)) + "\n")
        ...
        for repo_contributor in contributors:
            try:
                ...
                #   `created at`
                #   i think that's it
                cntrb_url = ("https://api.github.com/users/" +
                             repo_contributor['login'])
                ...
                cntrb = {
                    ...
                    "gh_url": contributor['url'],
                    "gh_html_url": contributor['html_url'],
                    # This is what we are dup checking
                    "gh_node_id": contributor['node_id'],
                    "gh_avatar_url": contributor['avatar_url'],
                    "gh_gravatar_id": contributor['gravatar_id'],
                    "gh_followers_url": contributor['followers_url'],
                ...
                if repo_contributor['flag'] == 'need_update':
                    ...
                    self.logger.info(
                        "Updated tuple in the contributors table with existing email: {}".format(email))
                ...
                self.logger.error(
                    "Cascading Contributor Anomalie from missing repo contributor data: {} ...\n".format(cntrb_url))
                continue
    ...
    def query_github_contributors_bulk(self, entry_info, repo_id):
        ...
        for repo_contributor in source_contributors['insert']:
            ...
            cntrb_url = (
                f"https://api.github.com/users/{repo_contributor['login']}")
        ...
        contributors_insert_result, contributors_update_result = self.bulk_insert(self.contributors_table,
                                                                                  update=source_contributors[
                                                                                      'update'], unique_columns=action_map['insert']['augur'],
    ...
    def query_gitlab_contributors(self, entry_info, repo_id):
        ...
        self.logger.info(
            "Querying contributors with given entry info: " + str(entry_info) + "\n")
        ...
        # list to hold contributors needing insertion or update
        contributors = self.paginate("https://gitlab.com/api/v4/projects/" + url_encoded_format +
                                     "/repository/contributors?per_page=100&page={}", duplicate_col_map, update_col_map, table, table_pkey, platform='gitlab')

        for repo_contributor in contributors:
            try:
                cntrb_compressed_url = (
                    "https://gitlab.com/api/v4/users?search=" + repo_contributor['email'])
                self.logger.info("Hitting endpoint: " +
                                 cntrb_compressed_url + " ...\n")
                r = requests.get(url=cntrb_compressed_url,
                ...
                self.logger.info("Fetching for user: " +
                                 str(contributor_compressed[0]["id"]))

                cntrb_url = ("https://gitlab.com/api/v4/users/" +
                             str(contributor_compressed[0]["id"]))
                self.logger.info(
                    "Hitting end point to get complete contributor info now: " + cntrb_url + "...\n")
                r = requests.get(url=cntrb_url, headers=self.headers)
                contributor = r.json()
    ...
    def update_gitlab_rate_limit(self, response, bad_credentials=False, temporarily_disable=False):
        ...
        if self.oauths[0]['rate_limit'] <= 0:
            try:
                ...
                self.logger.info(
                    f"Could not get reset time from headers because of error: {e}")
                reset_time = 3600
            time_diff = datetime.datetime.fromtimestamp(
                int(reset_time)) - datetime.datetime.now()
            self.logger.info(
                "Rate limit exceeded, checking for other available keys to use.")
            ...
            for oauth in other_oauths:
                ...
                if oauth['rate_limit'] > new_oauth['rate_limit']:
                    self.logger.info(
                        f"Higher rate limit found in oauth: {oauth}")
                    new_oauth = oauth
                elif (
    ...
    def update_gh_rate_limit(self, response, bad_credentials=False, temporarily_disable=False):
        ...
        if self.oauths[0]['rate_limit'] <= 0:
            ...
            for oauth in other_oauths:
                ...
                if oauth['rate_limit'] > new_oauth['rate_limit']:
                    self.logger.info(
                        "Higher rate limit found in oauth: {}\n".format(oauth))
                    new_oauth = oauth
                elif (
                    oauth['rate_limit'] == new_oauth['rate_limit']
                    and oauth['seconds_to_reset'] < new_oauth['seconds_to_reset']
    ...
    def paginate_endpoint(
        ...
        table_values = self.db.execute(
            s.sql.select(self.get_relevant_columns(
                table, action_map)).where(where_clause)
        ).fetchall()
        ...
        while True:
            ...
            if 'last' in response.links and last_page_number == -1:
                if platform == 'github':
                    last_page_number = int(
                        response.links['last']['url'][-6:].split('=')[1])
                elif platform == 'gitlab':
                    last_page_number = int(
                        response.links['last']['url'].split('&')[2].split('=')[1])
            ...
            if (page_number <= 1 and not forward_pagination) or \
                ...
                self.logger.info(
                    "No more pages to check, breaking from pagination.\n")
                break
        ...
        if forward_pagination:
            need_insertion, need_update = self.organize_needed_data(
                all_data, table_values, list(table.primary_key)[
                    0].name, action_map
            )
    ...
    def paginate(self, url, duplicate_col_map, update_col_map, table, table_pkey, where_clause="", value_update_col_map={}, platform="github"):
        ...
        update_keys += list(value_update_col_map.keys()
                            ) if value_update_col_map else []
        cols_to_query = list(duplicate_col_map.keys()) + \
            update_keys + [table_pkey]
        table_values = self.get_table_values(
            cols_to_query, [table], where_clause)
        ...
        while True:
            ...
            while num_attempts < 3:
                ...
                if 'last' not in r.links:
                    ...
                    if platform == "github":
                        ...
                        last_page = r.links['last']['url'].split('&')[
                            2].split("=")[1]
                    self.logger.info("Analyzing page {} of {}\n".format(
                        i, int(last_page) + 1 if last_page is not None else '*last page not known*'))
                ...
                if type(j) != dict and type(j) != str:
                    ...
                    if j['message'] == 'You have triggered an abuse detection mechanism. Please wait a few minutes before you try again.':
                        ...
                        self.update_rate_limit(
                            r, temporarily_disable=True, platform=platform)
                    if j['message'] == 'Bad credentials':
                        self.logger.info("rate limit update code goes here")
                        self.update_rate_limit(
                            r, bad_credentials=True, platform=platform)
                elif type(j) == str:
            ...
            if 'last' in r.links and not multiple_pages and not self.finishing_task:
                if platform == "github":
                    ...
                    i = int(r.links['last']['url'].split(
                        '&')[2].split("=")[1]) + 1
                self.logger.info(
                    "Multiple pages of request, last page is " + str(i - 1) + "\n")
                multiple_pages = True
            ...
            # Checking contents of requests with what we already have in the db
            j = self.assign_tuple_action(
                j, table_values, update_col_map, duplicate_col_map, table_pkey, value_update_col_map)

            if not j:
                self.logger.error(
                    "Assigning tuple action failed, moving to next page.\n")
                i = i + 1 if self.finishing_task else i - 1
                continue
            try:
                to_add = [obj for obj in j if obj not in tuples and (
                    obj['flag'] != 'none')]
            except Exception as e:
                self.logger.error(
                    "Failure accessing data of page: {}. Moving to next page.\n".format(e))
                i = i + 1 if self.finishing_task else i - 1
                continue
            if len(to_add) == 0 and multiple_pages and 'last' in r.links:
                ...
                if platform == "github":
                    page_number = int(
                        r.links['last']['url'][-6:].split('=')[1])
                elif platform == "gitlab":
                    page_number = int(
                        r.links['last']['url'].split('&')[2].split("=")[1])
                if i - 1 != page_number:
                    self.logger.info(
                        "No more pages with unknown tuples, breaking from pagination.\n")
                    break
