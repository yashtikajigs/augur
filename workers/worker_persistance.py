...
class Persistant():
    ...
    def __init__(self, worker_type, data_tables=[], operations_tables=[]):
        ...
        try:
            worker_defaults = self.augur_config.get_default_config(
            )['Workers'][self.config['worker_type']]
            self.config.update(worker_defaults)
        except KeyError as e:
            logging.warn('Could not get default configuration for {}'.format(
                self.config['worker_type']))

        worker_info = self.augur_config.get_value(
            'Workers', self.config['worker_type'])
    ...
    def initialize_logging(self):
        ...
        server_logfile = logfile_dir + \
            '{}_{}_server.log'.format(self.worker_type, self.config["port"])
        collection_logfile = logfile_dir + \
            '{}_{}_collection.log'.format(
                self.worker_type, self.config["port"])
    ...
    def initialize_database_connections(self):
        DB_STR = 'postgresql://{}:{}@{}:{}/{}'.format(
            self.config['user_database'], self.config['password_database'], self.config[
                'host_database'], self.config['port_database'], self.config['name_database']
        )
    ...
    def sync_df_types(self, subject, source, subject_columns, source_columns):
        ...
        for index in range(len(source_columns)):
            ...
            try:
                source_index = source_columns[index]
                type_dict[subject_columns[index]] = type(
                    source[source_index].values[0])
                ...
                self.logger.info(
                    f"Source data registered exception: {source[source_index]}")
    ...
    def _setup_postgres_merge(self, data_sets, sort=False):
        ...
        for index, data in enumerate(data_sets):
            ...
            data_table = s.schema.Table(
                f"merge_data_{index}_{os.getpid()}", metadata)
                ...
                data_table.append_column(
                    s.schema.Column(
                        column, self.get_sqlalchemy_type(
                            df.fillna(
                                method='bfill').iloc[0][column], column_name=column
                        )
                    )
                )
    ...
    def _get_data_set_columns(self, data, columns):
        ...
        for column in columns:
            ...
            try:
                ...
                self.logger.info(
                    "Columns have already been added, moving on...")
                pass
    ...
    def organize_needed_data(
        ...
        if not in_memory:
            ...
            (new_data_table, table_values_table), metadata, session = self._setup_postgres_merge(
                [
                    ...
                    self._get_data_set_columns(
                        table_values, table_value_columns)
                ]
            )
            ...
            need_insertion = pd.DataFrame(session.query(new_data_table).join(table_values_table,
                ...
                table_values_table.c[action_map['insert']
                                     ['augur'][0]] == None
            ...
            new_data_df = pd.DataFrame(new_data).dropna(
                subset=action_map['insert']['source'])
            ...
            if 'update' in action_map:
                ...
                while attempts < 50:
                    try:
                        ...
                        for sub_df in numpy.array_split(new_data_df, partitions):
                            self.logger.info(
                                f"Trying a partition, len {len(sub_df)}\n")
                            ...
                            self.logger.info(
                                f"need_updates merge: {len(sub_df)} worked\n")
                        break
                ...
                if attempts >= 50:
                    ...
                    need_updates = need_updates.drop(
                        [column for column in action_map['insert']['augur']], axis='columns')
    ...
    def assign_tuple_action(self, new_data, table_values, update_col_map, duplicate_col_map, table_pkey, value_update_col_map={}):
        ...
        for i, obj in enumerate(new_data):
            ...
            for augur_col, value_check in value_update_col_map.items():
                not_nan_check = not (math.isnan(value_check) and math.isnan(
                    existing_tuple[augur_col])) if value_check is not None else True
                if existing_tuple[augur_col] != value_check and not_nan_check:
                    continue
                self.logger.info(
                    "Found a tuple that needs an update for column: {}\n".format(augur_col))
    ...
    def bulk_insert(
        ...
        if len(insert) > 0:
            ...
            # convert list of json records that need inserted to pandas dataframe
            df = pd.DataFrame(insert)

            # declare conn used for transaction
            conn = self.db.connection

            # create list of columns for insertion
            table_columns = df.columns

            # change data type of float nans to int
            if convert_float_int:
                df = self._convert_float_nan_to_int(df)

            # setup buffer for values to be stored
            s_buf = StringIO()
            ...
            # sets the cursor to index 0 in the file
            s_buf.seek(0)
            ...
            self.logger.info(
                ...
                "thanks to psycopg2 copy_from csv! :)"
            ...
            #         self.logger.debug(
            #             f'table name is: {table_name}, and columns are {columns}.')
            ...
            # except Exception as e:
            #     self.logger.info(
            #         f"Bulk insert error 2: {e}. exception registered.")
        ...
        return [
            {
                **data_point,
                ...
                field: bytes(
                    data_point[field], "utf-8").decode("utf-8", "ignore").replace("\x00", "\uFFFD")
                # 0x00
    ...
    def _add_nested_columns(self, df, column_names):
        ...
        for column in column_names:
            ...
            self.logger.debug('\n')
            self.logger.debug(
                f'Expanded Columns Are:{expanded_column.columns}')
            self.logger.debug('\n')
            self.logger.debug('\n')
            self.logger.debug('\n')
            ...
            try:
                ...
                self.print_traceback(
                    "value error in _add_nested_columns", e, True)
    ...
    def enrich_data_primary_keys(
        ...
        source_df=self._add_nested_columns(
            pd.DataFrame(source_data), gh_merge_fields)
        ...
        if not in_memory:
            ...
            all_primary_keys=self.db.execute(s.sql.select(
                [table.c[field] for field in augur_merge_fields] +
                [table.c[list(table.primary_key)[0].name]]
            )).fetchall()
            ...
            while attempts < 50:
                try:
                    ...
                    for sub_df in numpy.array_split(source_df, partitions):
                        ...
                        self.logger.info(
                            f"source_pk merge: {len(sub_df)} worked\n")
                    break
            ...
            if attempts >= 50:
                ...
                self.logger.info(
                    f"Data enrichment successful, length: {len(source_pk)}\n")
    ...
    def new_organize_needed_data(
        ...
        self.logger.info(
            f"Beginning to organize needed data from {len(new_data)} data points...")
    ...
    def get_relevant_columns(self, table, action_map={}):
        columns=copy.deepcopy(
            action_map['update']['augur']) if 'update' in action_map else []
        ...
        try:
            relevant_columns_return=[
                table.c[column] for column in columns + [list(table.primary_key)[0].name]]
            return relevant_columns_return
        except Exception as e:
            self.logger.info(
                f"Column may not exist in the database -- registered exception: {e}.")
            self.print_traceback("", e, True)
    ...
    def retrieve_tuple(self, key_values, tables):
        ...
        values=json.loads(
            pd.read_sql(retrieveTupleSQL, self.db, params={}
