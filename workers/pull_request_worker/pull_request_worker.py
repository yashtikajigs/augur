...
class GitHubPullRequestWorker(WorkerGitInterfaceable):
    ...
    def multi_thread_urls(self, all_urls, max_attempts=5, platform='github'):
        ...
        def load_url(url, extra_data={}):
            try:
                ...
                self.logger.debug(
                    f"load_url inside multi_thread_urls failed with {e}, for usl {url}. exception registerred.registered")
        ...
        for urls in numpy.array_split(all_urls, partitions):
            attempts = 0
            self.logger.info(
                f"Total data points collected so far: {len(all_data)}")
            while len(urls) > 0 and attempts < max_attempts:
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    self.logger.info(
                        "Multithreaded urls and returned status codes:")
                    count = 0
                    for future in concurrent.futures.as_completed(future_to_url):
                        ...
                        try:
                            ...
                            # 403 is rate limit, 404 is not found, 401 is bad credentials
                                    ...
                                    if 'last' in response.links and "&page=" not in url[0]:
                                        urls += [
                                            (url[0] + f"&page={page}", extra_data) for page in range(
                                                2, int(
                                                    response.links['last']['url'].split('=')[-1]) + 1
                                            )
                                        ]
                                        # self.logger.info(f"urls boundry issue? for {urls} where they are equal to {url}.")

                                        urls = numpy.delete(
                                            urls, numpy.where(urls == url), axis=0)
                                except:
                                    self.logger.info(
                                        f"ERROR with axis = 0 - Now attempting without setting axis for numpy.delete for {urls} where they are equal to {url}.")
                                    urls = numpy.delete(
                                        urls, numpy.where(urls == url))
                                    continue
            ...
            while has_previous_page:
                ...
                for attempt in range(num_attempts):
                    ...
                    if 'data' in data:
                        ...
                        self.logger.info(
                            "Request returned a non-data dict: {}\n".format(data))
                        ...
                        if data['message'] == (
                            ...
                            num_attempts -= 1
                            self.update_gh_rate_limit(
                                response, temporarily_disable=True)
                        if data['message'] == "Bad credentials":
                            self.update_gh_rate_limit(
                                response, bad_credentials=True)
    ...
    def pull_request_files_model(self, task_info, repo_id):
        ...
        for index, pull_request in enumerate(pr_numbers.itertuples()):
            ...
            self.logger.info(
                f'Querying files for pull request #{index + 1} of {len(pr_numbers)}')
        ...
        table_values = pd.read_sql(table_values_sql, self.db, params={
                                   'repo_id': self.repo_id})
        ...
        if len(pr_file_rows) > 0:
            ...
            self.logger.info(
                f'No rows need insertion for repo {self.repo_id}\n')
            self.register_task_completion(
                task_info, self.repo_id, 'pull_request_files')
            return
    ...
    def pull_request_commits_model(self, task_info, repo_id):
        ...
        self.pr_meta_id_inc = self.get_max_id(
        ...
        for pull_request in urls.itertuples():  # for each url of PRs we have inserted
            ...
            pr_commits = self.paginate(
                ...
                where_clause="where pull_request_id = {}".format(
        ...
        self.register_task_completion(
            self.task_info, self.repo_id, 'pull_request_commits')

    def extract_pr_data(data):
    ...
    def _get_pk_source_prs(self):
        ...
        source_prs = self.paginate_endpoint_new(
            pr_url, table=self.pull_requests_table, extract_data_method=self.extract_pr_data, natural_key=natural_key)
        ...
        def pk_source_increment_insert(inc_source_prs, action_map):
            ...
            if len(inc_source_prs['all']) == 0:
                ...
                self.register_task_completion(
                    self.task_info, self.repo_id, 'pull_requests')
                return
            ...
            if len(inc_source_prs['insert']) > 0:
                ...
                self.logger.info(
                    "Contributor enrichment is not needed, no inserts in action map.")
            ...
            try:
                prs_insert = [
                    {
                        ...
                        'pr_augur_contributor_id': int(pr['cntrb_id']) if (  # Changed later on 12/3/2021 to use default contributor if something in enrich_cntrb_id broke
                            ...
                            # cast as an int because of an otherwise inexplicable error.
                            int(pr['cntrb_id'])
                ...
                self.print_traceback(
                    "Extracting data from source in pr model", e)
            ...
            if len(inc_source_prs['insert']) > 0 or len(inc_source_prs['update']) > 0:
                ...
                self.bulk_insert(
                    ...
                    insert=prs_insert, update_columns=[
                        'pr_src_state', 'pr_closed_at', 'pr_updated_at', 'pr_merged_at'],
                    convert_float_int=True
                )

                source_data = inc_source_prs['insert'] + \
                    inc_source_prs['update']
    ...
    def print_traceback(self, exception_message, exception, debug_log=True):
        ...
        if debug_log:
            self.logger.debug(
                f"{exception_message}. ERROR: {exception}", exc_info=sys.exc_info())
        else:
            self.logger.info(
                f"{exception_message}. ERROR: {exception}", exc_info=sys.exc_info())
        ...
        if pk_source_prs:
            try:
                ...
                try:
                    ...
                    try:
                        self.logger.info(
                            f"Pull request reviews model factored out for now due to speed.")
                    except Exception as e:
                        self.print_traceback(
                            "PR reviews model, which is factored out for now due to speed", e)
                    finally:
                        try:
                            self.pull_request_nested_data_model(pk_source_prs)
                            self.logger.info(
                                f"Pull request nested data model.")
                        except Exception as e:
                            self.print_traceback("PR nested model", e)
                        finally:
                            self.logger.debug(
                                "finished running through four models.")
    ...
    def pull_request_comments_model(self, pk_source_prs):
        ...
        def pr_comments_insert(inc_pr_comments, comment_action_map, comment_ref_action_map):
            ...
            inc_pr_comments['insert'] = self.text_clean(
                inc_pr_comments['insert'], 'body')
            # This is sending empty data to enrich_cntrb_id, fix with check
            if len(inc_pr_comments['insert']) > 0:
                ...
                self.logger.info(
                    "Contributor enrichment is not needed, no inserts in action map.")
            # added 12/3/2021 to put the value assignment into a try/except block
            pr_comments_insert = []
            try:
                pr_comments_insert = [
                    {
            ...
            try:
                ...
                try:
                    ...
                    self.write_debug_data(
                        c_pk_source_comments, 'c_pk_source_comments')

                    self.logger.info(
                        f"log of the length of c_pk_source_comments {len(c_pk_source_comments)}.")
                    ...
                    self.logger.info(
                        f"log of the length of both_pk_source_comments {len(both_pk_source_comments)}.")
   ...
   #  )
                    ...
                    self.logger.debug(
                        f"length of both_pk_source_comments: {len(both_pk_source_comments)}")
                    ...
                    self.print_traceback(
                        "Gathering and inserting data into pr mesaage ref table", e)
                finally:
        ...
        try:
                        ...
                        msg_row[0] for msg_row in self.db.execute(
                            s.sql.select(
                                ...
                                self.pull_request_message_ref_table.c.pull_request_id.in_(
                                    set(pd.DataFrame(pk_source_prs)
                                        ['pull_request_id'])
                                )
                            )
                        ).fetchall()
            ...
            pr_comments_insert(
                pr_comments, comment_action_map, comment_ref_action_map)
            self.logger.info(f"comments inserted for repo_id: {self.repo_id}")
            return
        except Exception as e:
            self.print_traceback(
                "Staggered paginate endpoint for pr comments", e)
        finally:
            self.logger.debug(
                f"Pull request messages and message refs worked without exception for {self.repo_id}")
    ...
    def pull_request_events_model(self, pk_source_prs=[]):
        ...
        if len(pk_pr_events) > 0:
            ...
            self.logger.info(
                "Contributor enrichment is not needed, no data provided.")
            ...
            while (pr_nested_loop < 5):
                try:
                    if pr_nested_loop == 1:
                        ...
                        self.bulk_insert(
                            self.pull_request_labels_table, insert=labels_insert)
                        ...
                        if len(source_reviewers_insert) > 0:
                            ...
                            self.logger.info(
                                "Contributor enrichment is not needed, no inserts provided.")
                        ...
                        self.bulk_insert(
                            self.pull_request_reviewers_table, insert=reviewers_insert)
                        ...
                        self.bulk_insert(
                            self.pull_request_assignees_table, insert=assignees_insert)
                        ...
                        if len(source_meta_insert) > 0:
                            ...
                            self.logger.info(
                                "Contributor enrichment is not needed, nothing in source_meta_insert.")
                        ...
                        self.bulk_insert(
                            self.pull_request_meta_table, insert=meta_insert)

                except Exception as e:
                    self.print_traceback(
                        f"Nested model error at loop {pr_nested_loop}", e)
                    continue
    ...
    def query_pr_repo(self, pr_repo, pr_repo_type, pr_meta_id):
        ...
        try:
            ...
            cols_query = list(duplicate_col_map.keys()) + \
            ...
            if new_pr_repo['owner'] and 'login' in new_pr_repo['owner']:
                cntrb_id = self.find_id_from_login(
                    new_pr_repo['owner']['login'])
            else:
                cntrb_id = 1
            ...
            if new_pr_repo['flag'] == 'need_insertion':
                ...
                self.logger.info(
                    f"Added PR {pr_repo_type} repo {result.inserted_primary_key}")
